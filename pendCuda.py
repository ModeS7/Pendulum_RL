...
def debug_policy(policy, env, steps=10):
    ...
    print("\nPolicy outputs for different states:")
    for i in range(steps):
        ...
        print(
            f"Step {i}: State=[{td['th'].item():.4f}, {td['thdot'].item():.4f}, {td['phi'].item():.4f}, {td['phidot'].item():.4f}], Action={action:.4f}")

        # Apply random action to get to different state
